...
### Stories and Rules for Interjections

You can't revert every kind of interjection. In these cases,
you need stories or rules with interruptions in them. 
...
You can then write rules for specific responses to interruptions based on the value of `requested_slot`, for example:

```
rule:

- rule: email interruption
  condition:
  - slot_was_set: 
